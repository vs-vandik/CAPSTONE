"""Smoke test: drive a short discourse with Thiel as one of the experts and
print every turn plus the quotes that the retriever surfaced.

Run from `backend/`:
    python -m scripts.smoke_thiel
"""
...
def main() -> int:
    ...
    session = {
        "topic": (
            "Is technological progress meaningfully slowing down, "
            "and what should investors do about it?"
        ),
        "persona_ids": ["fink", "thiel"],
        "max_turns": 2,
        "history": [],
        "status": "active",
    }
